...
use bitsafe_common::socket;
use bitsafe_protocol::codec::{handshake_client, read_message, write_message};
...
#[derive(Parser)]
#[command(name = "bitsafe", about = "BitSafe password manager CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    /// Environment values matching "bitsafe:<id>/<field>" are replaced with
...
fn handle_service_action(action: ServiceAction) -> Result<()> {
    match action {
        ServiceAction::SshSocket => {
            println!("{}", bitsafe_common::socket::ssh_agent_socket_path().display());
            Ok(())
        }
...
fn install_service() -> Result<()> {
    ...
    let service_bin = exe.with_file_name("bitsafe-service");

    if !service_bin.exists() {
        anyhow::bail!(
            "bitsafe-service not found at {}. Install it first.",
            service_bin.display()
        );
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let unit_path = unit_dir.join("bitsafe.service");
        let unit = format!(
            "[Unit]\n\
             ...
             Restart=on-failure\n\
             RestartSec=5\n\
             Environment=SSH_AUTH_SOCK=%t/bitsafe/ssh-agent.sock\n\
             \n\
             [Install]\n\
        ...
        println!("  export SSH_AUTH_SOCK=\"${{XDG_RUNTIME_DIR}}/bitsafe/ssh-agent.sock\"");
        Ok(())
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let plist_path = plist_dir.join("com.bitsafe.service.plist");
        let ssh_sock = bitsafe_common::socket::ssh_agent_socket_path();
        let plist = format!(
...
<dict>
    <key>Label</key>
    <string>com.bitsafe.service</string>
...
fn uninstall_service() -> Result<()> {
    #[cfg(target_os = "linux")]
    {
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "disable", "--now", "bitsafe"])
            .status();

        let unit_path = dirs::config_dir()
            .context("Cannot determine config dir")?
            .join("systemd/user/bitsafe.service");
        if unit_path.exists() {
    ...
    #[cfg(target_os = "macos")]
    {
        let plist_path = dirs::home_dir()
            .context("Cannot determine home dir")?
            .join("Library/LaunchAgents/com.bitsafe.service.plist");
...
async fn send_request(request: Request) -> Result<Response> {
    ...
    let stream = UnixStream::connect(&socket_path)
        ...
        .with_context(|| {
            format!(
                "Failed to connect to bitsafe-service at {}.\nIs the service running?",
                socket_path.display()
            )
        })?;
...
/// `bitsafe run -- <command>`: resolve vault references in env vars, then exec.
async fn handle_run(command: Vec<String>) -> Result<()> {
    use bitsafe_protocol::request::{ResolveRefsParams, VaultRef};
    use bitsafe_protocol::response::ResolvedRef;
    use std::ffi::CString;

    if command.is_empty() {
        anyhow::bail!("No command specified. Usage: bitsafe run -- <command> [args...]");
    }

    // Scan environment for bitsafe: references
    let mut refs_to_resolve: Vec<(String, VaultRef)> = Vec::new(); // (env_key, ref)

    for (key, value) in std::env::vars() {
        if let Some(vault_ref) = parse_bitsafe_ref(&value) {
            refs_to_resolve.push((key, vault_ref));
        }
    }

    if refs_to_resolve.is_empty() {
        tracing::debug!("No bitsafe: references found in environment, exec directly");
    } else {
        ...
        let resolve_request = Request::new(
            1,
            bitsafe_protocol::request::methods::VAULT_RESOLVE_REFS,
        ...
        for (i, result) in resolved.iter().enumerate() {
            if let Some(err) = &result.error {
                ...
                eprintln!("bitsafe: failed to resolve {env_key}: {err}");
                had_errors = true;
            }
        }
        if had_errors {
...
/// Parse a "bitsafe:<id>/<field>" or "bitsafe://<name>/<field>" reference from an env value.
fn parse_bitsafe_ref(value: &str) -> Option<bitsafe_protocol::request::VaultRef> {
    let rest = value.strip_prefix("bitsafe:")?;
    ...
    Some(bitsafe_protocol::request::VaultRef {
        id: id.to_string(),
...
mod tests {
    ...
    #[test]
    fn parse_bitsafe_ref_id_and_field() {
        let r = parse_bitsafe_ref("bitsafe:64b18d6b/password").unwrap();
        assert_eq!(r.id, "64b18d6b");
        assert_eq!(r.field, "password");
    }

    #[test]
    fn parse_bitsafe_ref_full_uuid() {
        let r = parse_bitsafe_ref("bitsafe:64b18d6b-8161-4a0c-befb-c3484d36ec68/username").unwrap();
        assert_eq!(r.id, "64b18d6b-8161-4a0c-befb-c3484d36ec68");
        assert_eq!(r.field, "username");
    }

    #[test]
    fn parse_bitsafe_ref_name_lookup() {
    ...
    #[test]
    fn parse_bitsafe_ref_not_a_ref() {
        assert!(parse_bitsafe_ref("just a normal value").is_none());
        assert!(parse_bitsafe_ref("bitsafe:").is_none());
        assert!(parse_bitsafe_ref("bitsafe:id-only").is_none());
        assert!(parse_bitsafe_ref("bitsafe:/no-field").is_none());
    }
}
