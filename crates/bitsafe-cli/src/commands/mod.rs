...
use bitsafe_protocol::response::{Response, StatusResult, TotpResult, VaultItem, VaultItemDetail};
...
pub fn handle_login(response: Response, json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("Logged in successfully. Vault is locked — run `bitsafe unlock` to decrypt.");
    }
    Ok(())
}
