# ADR 007: Secret Injection via `grimoire run`
...
We want `grimoire run -- <command>` to inject secrets from the vault into environment variables, while keeping the child process as close to a direct terminal execution as possible.
...
```bash
# Environment variables with grimoire: prefix are replaced
export DATABASE_URL="grimoire:64b18d6b-8161-4a0c-befb-c3484d36ec68/password"
grimoire run -- ./my-app
...
# Multiple secrets
API_KEY="grimoire:abc123/password" DB_PASS="grimoire:def456/notes" grimoire run -- ./deploy.sh
```
...
|-----------|-------------|
| `grimoire:` | Prefix identifying a vault reference |
| `<item-id>` | Cipher UUID (full or shortened prefix, minimum 6 chars) |
| `<field>` | Field to extract: `password`, `username`, `uri`, `notes`, `totp`, `name` |

Short IDs are resolved by prefix match — `grimoire:64b18d` matches `64b18d6b-8161-4a0c-befb-c3484d36ec68`. Ambiguous matches are an error.

#### Name-based references

```
grimoire://GitHub/password
grimoire://My Database/username
```
...
2. **`exec` semantics**: After resolving secrets and setting environment variables, `grimoire run` uses `exec` to replace itself with the child process. There is no wrapper process sitting between the shell and the child.
...
grimoire run -- ./my-app
         │
         ├── 1. Scan environ for values matching "grimoire:..." pattern
         ...
         │
         └── (grimoire process is gone — child IS the process now)
```

Step 5 is critical: we call `execvp()`, not `fork()+exec()`. The `grimoire` process ceases to exist. The child process owns the TTY, the PID, everything.

### Secret Lifetime

- Secrets exist only in the child's environment (in-memory, managed by the kernel)
- Not written to disk
- Not visible in `grimoire` process (it's gone after exec)
...
| No `grimoire:` refs in environ | Warn and exec anyway (no-op) |
...
```bash
# Load from a .env file containing grimoire: references
grimoire run --env-file .env.secrets -- ./my-app
```
...
- **Minimal attack surface**: The `grimoire` process doesn't exist after exec. Secrets are only in the child's environ.
...
- **Service must be unlocked**: The vault must be unlocked before `grimoire run` works. If locked, we trigger an unlock prompt, which may be unexpected in CI contexts. Use `prompt.method = "none"` in config for CI, and pass secrets via `--env` flags directly.
