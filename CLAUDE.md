...
- **`grimoire run` uses exec** — after resolving `grimoire:<id>/<field>` references in env vars, the process is replaced via `execvp()`. No wrapper process, no TTY breakage. All resolution errors must happen before exec. `--manifest` flag adds declarative `.env.grimoire` manifest files (grimoire refs only, no plain values). Existing env vars override manifest entries. See `docs/secret-injection.md` and `specs/007-secret-injection.md`, `specs/015-env-manifest.md`.
- **`grimoire clip`** — copies a vault secret to the system clipboard with fixed 15s auto-clear. Uses SHA-256 hash comparison to avoid clearing user's subsequent clipboard contents. Spawns background clearer via `current_exe()` (no PATH lookup). Secret wrapped in `Zeroizing`. See `specs/012-clipboard-auto-clear.md`.
- **`grimoire generate`** — password/passphrase generation using `OsRng` (OS CSPRNG). Bundled EFF large wordlist (7776 words) via `include_str!`. Entropy estimate to stderr. No service interaction. See `specs/013-password-generation.md`.
- **`grimoire credential-helper`** — git credential helper protocol implementation. HTTPS only, URI-based matching with 3-tier priority (exact > host+path prefix > host-only), read-only v1 (store/erase are no-ops). Binary name detection: when invoked as `git-credential-grimoire` (via symlink), routes to credential helper logic. See `specs/014-git-credential-helper.md`.
...
- **Offline vault cache (ADR 016)** — unlock is cache-first: if `~/.local/share/grimoire/vault_cache.bin` exists, the service unlocks from the local cache without contacting the server. Cache is envelope-encrypted with a CEK stored in the OS credential store (macOS Keychain / Linux Secret Service). HMAC-SHA256 (keyed by HKDF from password hash) provides password verification + integrity. Cache is built at login, read-only at unlock, and updated by sync when vault hash changes. Logout deletes cache + CEK. Password changes on another device require `grimoire logout` + `grimoire login`.
- **Cache file format** — binary: `[GRMC magic][version u32 LE][flags u8][vault_hash 32B][nonce 24B][body_len u32 LE][body]`. Body is XChaCha20-Poly1305 ciphertext (if CEK) or raw JSON (fallback). KDF floor enforced on cache read: PBKDF2 ≥ 100K iter, Argon2id ≥ 3/64/1.
- **Sync returns raw cipher JSON** — `SyncResult.raw_ciphers` carries the patched cipher JSON for cache updates. `populate_from_cache()` replays cached ciphers into the SDK repo. 401/403 from sync triggers `SdkError::AuthRevoked` → cache + CEK deletion + vault lock.
